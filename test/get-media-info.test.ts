...
describe("getMediaInfo with real files", () => {
  ...
  describe("Consistency check", () => {
    it("should return consistent MediaInfo for .mp4 file between parsers", async () => {
      ...
      expect(mp4boxInfo.videoStreams.length).toBe(
        remotionInfo.videoStreams.length,
      );
      expect(mp4boxInfo.audioStreams.length).toBe(
        remotionInfo.audioStreams.length,
      );

      // Duration might slightly differ due to timescale
      expect(
        Math.abs(
          (mp4boxInfo.durationInSeconds || 0) -
            (remotionInfo.durationInSeconds || 0),
        ),
      ).toBeLessThan(0.1);
      ...
      // Dimensions
      expect(mp4boxInfo.videoStreams[0].width).toBe(
        remotionInfo.videoStreams[0].width,
      );
      expect(mp4boxInfo.videoStreams[0].height).toBe(
        remotionInfo.videoStreams[0].height,
      );
    });
  });
});
