...
describe('getMediaInfo with auto parser (default)', () => {
  const fixtures: Record<string, 'mp4box' | 'isoboxer' | 'remotion' | 'media-utils'> = {
    ...
    'engine-start.h264.pcms16le.avi': 'media-utils',
    'engine-start.mpeg2video.mp2.m2ts': 'media-utils',
    'engine-start.h264.aac.m2ts': 'media-utils',
    'engine-start.h264.mp3.m2ts': 'media-utils',
  };
  ...
  // it.each([])('should fail to parse %s', async (filename) => {
  //   try {
