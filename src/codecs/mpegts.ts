...
import { BitReader, readAscii, toHexString } from './binary';
...
export function parseAacHeaderInPES(data: Uint8Array, streamInfo: Partial<AudioStreamInfo>) {
  ...
  try {
    ...
    console.error('aac is ffmpeg latm', info);
    ...
    streamInfo.codec = 'aac_latm';
    streamInfo.codecDetail = 'AAC in FFMPEG LATM';
    return;
  } catch (error) {
    // ignore, fall back to raw AAC
    console.error('Invalid FFMPEG LATM header inside AAC PES', error);
  }
 ...
 */
export function parseMp2OrMp3HeaderInPES(data: Uint8Array, streamInfo: Partial<AudioStreamInfo>) {
  if (data.length < 5) {
